...
class ConnectedGrid extends Component {
  ...
  handleAddClick = () => {
    ...
    this.props.dispatch(postSomething(body)).then(res => {
      if (!this.props.items.includes(res.name)) {
        return this.fetchData()
      }
    })
  }
  ...
  handleDeleteClick = deleteTarget => {
    this.props.dispatch(deleteSomething(deleteTarget.url)).then(() => {
      if (this.props.items.includes(deleteTarget)) {
        return this.fetchData()
      }
    })
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <StyledSmallText>{this.props.submitStatus}</StyledSmallText>
        <StyledGrid>
          {this.props.items
            .sort((a, b) =>
